...
```
> 이전 코드에서 현재 인원이 점검할 수 있는 위치를 계산하는 과정에서 조건을 잘못 설정하여 많은 문제가 발생했음을 알아채고 조건을 정확하게 수정했다. 하지만 그 과정에서 분기가 하나 추가되어서 그런지 이전에 발생하지 않았던 시간 초과가 발생한 케이스가 생겨버렸다. 시간을 줄일수 있는 방법을 고민해봐야 할 것 같다.
